...
client.on('ready', () => {
 ...
 client.user.setGame('Txe'Security.','https://www.twitch.tv/peery13');
  console.log('')
  console.log('')
  console.log('╔[═════════════════════════════════════════════════════════════════]╗')
...
client.on("message", message => {
if(message.content === ".") {
message.channel.send("**!-# Welcome'To,Txee :Txe: :dove: **");
}
});

client.on('guildMemberAdd', member => {
    let channel = member.guild.channels.find('name', 'txe');
    let memberavatar = member.user.avatarURL
      if (!channel) return;
    let embed = new Discord.RichEmbed()
        .setColor('RANDOM')
...
client.on ("guildMemberAdd", member => {
   ...
   var role = member.guild.roles.find ("name", "- Txe .'!");
   member.addRole (role);
